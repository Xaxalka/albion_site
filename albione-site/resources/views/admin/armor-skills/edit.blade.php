@extends('layouts.app')
@php
    $title = 'Edit Armor Skill';
@endphp

@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900">Edit Armor Skill for {{ $armor->name }}</h1>
        <p class="text-sm text-slate-600">Manage armor abilities and effects.</p>
    </div>

    <form action="{{ route('admin.armor-skills.update', $skill->id) }}" method="POST" class="space-y-4 rounded-lg border border-slate-200 bg-white p-6">
        @csrf
        @method('PUT')
